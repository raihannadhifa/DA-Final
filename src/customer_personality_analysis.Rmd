...
# Initial Hypothesis
Here are some initial hypothesis of what this dataset could answer:
1. Does age and education level matter to the customer's shopping preferences?
2. Does customer buy more products the longer they associated with the company?
3. Does marital status and number of kids matter on the customer's shopping behavior?

We will try to find the answer to this hypothesis by doing analysis on the dataset.
...
Gower distance is computed as the average of partial dissimilarities across individuals. Each partial dissimilarity (and thus Gower distance) ranges in [0 1]. To calculate this distance, we use cluster::daisy() instead of stats::dist().
...
8. test outliers. There's one observation with disproportionately large Income 666666, we estimate this to be an outlier. 
```{r, warning = FALSE}
ggplot(data = data, aes(x=Income)) +
  geom_boxplot()
data[data$Income>4e05,'Income']
```

9. Thus, we are left with 12 variables that can be used to create the distance matrix.
```{r, warning = FALSE}
data_12var <- data[c(1, 30, 3, 4, 5, 33, 32, 9, 34, 35, 16, 20)] %>% 
  tibble() %>% 
  filter(Income < 4e05)
```
...
## Charts
1. Age
```{r age_distribution, warning = FALSE}
ggplot(data = data_12var, aes(Age)) +
  geom_density()
```

2. Education 
```{r education_distribution, warning = FALSE}
ggplot(data = data_12var, aes(Education)) +
  geom_histogram(stat = "count")
```

```{r age_distribution_by_education, warning = FALSE}
ggplot(data = data_12var, aes(Age, fill = Education)) + 
  geom_histogram()
```

3. Marital Status 
```{r marital_status_distribution, warning = FALSE}
ggplot(data = data_12var, aes(Marital_Status, fill = Marital_Status)) +
  geom_histogram(stat = "count")
```

4. Income & Amount Spent
The total amount spent seems to be linear related to the square of Income
```{r income_spent_relationship, warning = FALSE}
ggplot(data = data_12var, aes(Income^2, Amt_Spent)) +
  geom_point(alpha = 0.5, color = "blue") + 
  scale_x_continuous(labels = comma)
```

5. Child 
```{r child_distribution_by_marital, warning = FALSE}
ggplot(data = data_12var,  aes(Child, fill = Marital_Status)) + 
  geom_histogram(position = "dodge")
```

6. Customer Seniority & Amount Spent 
```{r spend_distribution_by_seniority, warning = FALSE}
ggplot(data = data_12var, 
       aes(as.factor(Customer_Seniority), Amt_Spent, fill = Customer_Seniority)) + 
  geom_boxplot(color = "black")
```

# Analysis
## Analysis based on income, seniority, age, and amount spent
```{r distance_matrix, warning = FALSE}
gower_dist2 <-
  daisy(data_12var[c("Income", "Amt_Spent", "Customer_Seniority", "Age")],
        metric = "gower")
...
### Most similar pair
```{r most_similar, warning = FALSE}
data_12var[which(gower_mat2 == min(gower_mat2[gower_mat2 != min(gower_mat2)]),
           arr.ind = TRUE)[1,],]
```

### Most dissimilar pair
```{r most_dissimilar, warning = FALSE}
data_12var[which(gower_mat2 == max(gower_mat2[gower_mat2 != max(gower_mat2)]),
           arr.ind = TRUE)[1,],]
```
